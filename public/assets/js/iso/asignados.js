...
$(document).ready(function() {
    ...
    var table = $("#TablaDocumentos").DataTable({
        ...
        order: [[4, 'asc']], // Columna
        ajax: {
            url: BASE_URL + "/app/ajax/iso/06_ajax_distribucionISO.php", 
            method: "POST",
            dataSrc: "",
            data: function(d) {
                // Obtiene el valor seleccionado en el combo de perfil
                d.var_perfil = $('#perfil').val(); 
            }
        },
        deferRender: true,
        columns: [
            ...
            {data: "transversal", title: 'Transv.' , className: 'text-start text-bold'},
            {data: "responsable", title: 'Responsable' , className: 'text-start text-bold'},
            {data: "modo_almacenamiento", title: 'Almacenamiento' , className: 'text-start text-bold'},
            {data: "tiempo_retencion", title: 'Retención' , className: 'text-start text-bold'},
            {data: "recuperacion", title: 'Recuperación' , className: 'text-start text-bold'},
            {data: "disposicion", title: 'Disposición' , className: 'text-start text-bold'},
            {data: "nombre_archivo", title: 'Ver', class: 'clases_para_el_td', render: function(data, type, row) {
                var d = {};
        ...
        columnDefs: [
            { targets: -1 , // mantener la última columna (Ver) con un ancho fijo
                data: "nombre_archivo",
        ...
        // Actualiza la tabla cada vez que se selecciona un nuevo proceso
