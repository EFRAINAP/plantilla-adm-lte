$(document).ready(function() {
	...
	var table = $("#TablaDocumentos").DataTable({
		...
		order: [[4, 'asc']], // Columna
		ajax: {
			url: BASE_URL + "/app/ajax/iso/06_ajax_documentosISO.php", 
			method: "POST",
			dataSrc: "",
		},
		deferRender: true,
		columns: [
			...
			{data: "tipo_documento", title: 'Tipo', className: 'text-start text-bold'},
			{data: "cod_documento", title: 'Código', className: 'text-start text-bold'},
			{data: "descripcion_documento", title: 'Descripción', className: 'text-start text-bold'},
			...
			{data: null, title: 'Acciones', render: function(data, type, row) {  
				return '<a href="05_EditarDocumentos.php?cod_documento=' + row.cod_documento + '" class="btn btn-warning"> <i class="bi bi-pencil"></i></a>' +
						'<a href="06_EliminarDocumentos.php?cod_documento=' + row.cod_documento + '" class="btn btn-danger"> <i class="bi bi-trash"></i></a>';}
			},
            {data: "nombre_archivo", title: 'Nombre Archivo', className: 'text-start text-bold', render: function(data, type, row) {
				...
				if (row.visualizacion === 'Lectura') {
					return '<a href="' + BASE_URL + '/visor?id=' + row.cod_documento + '&tipo=documento&categoria=iso" class="btn btn-danger btn-sm" type="button" style="margin: 1px" title="Mcostrar" target="_blank">Lectura <i class="bi bi-file-pdf"></i></a>';
				} 
		...
		columnDefs: [
			{targets: -1, data: null, responsivePriority: 1},
            {targets: -2, data: "nombre_archivo", responsivePriority: 2}
		],
		dom: "Brtip",
		buttons: [
