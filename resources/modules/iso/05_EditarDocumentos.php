<?php
	...
	// Verificar si el usuario puede editar documentos (administradores tienen acceso automático)
	if (!has_access_with_permissions($name_page_principal, $permiso)) {
		echo json_encode(['error' => true, 'message' => 'No tiene permisos para editar documentos.']);
...
<?php
	...
	if(isset($_POST['editar_documento'])){
		...
		$req_fields = array('area', 'proceso', 'responsable', 'tipodocumento', 'descripciondocumento', 'version', 'fecha', 'frecuencia', 'transversal', 'visualizacion', 'estado_documento', 'cod_documento');
		...
		$query = "UPDATE documentos SET 
					...
					descripcion_documento 	= '{$descripciondocumento}', 
					version 				= '{$version}', 
					fecha 					= '{$fecha}',			
		...
		if(isset($_FILES['doc_adjunto']['name']) && $_FILES['doc_adjunto']['error'] === UPLOAD_ERR_OK){
			...
			// Obtener el nombre del archivo anterior desde la base de datos
			$previous_file_path = $upload_path . $documentos['nombre_archivo'];
			// Eliminar el archivo anterior si existe
			if (file_exists($previous_file_path)) {
				unlink($previous_file_path); // Eliminar el archivo anterior
			}
			// Mover el archivo al servidor
			if(move_uploaded_file($_FILES['doc_adjunto']['tmp_name'], $file_path)){
				// Añadir detalles del archivo a la consulta
				$query .= ", nombre_archivo = '{$nombrearchivo}'";
			} else {
				$session->msg('d', 'Error al mover el archivo.');
				redirect('05_EditarDocumentos.php?cod_documento='.$documentos['cod_documento'], false);
				exit;
			}
		}
		// Ejecutar la actualización de la tabla (actualiza siempre, ya sea con o sin archivo)
		$query .= " WHERE cod_documento = '{$documentos['cod_documento']}'";
		// Ejecutar la consulta
		if($db->query($query) && $db->affected_rows() == 1){
			$session->msg('s', "Registro ha sido actualizado.");
			redirect('03_AdministrarDocumentos.php', true);
		} else {
	...
	<div class="col-md-12">
		<div class="panel panel-default">	
			...
			<div class="panel-body" style="background-color: #ECF0F1">
				 <form method="post" action="05_EditarDocumentos.php?cod_documento=<?php echo (string)$documentos['cod_documento']; ?>" enctype="multipart/form-data">             
					<div class="row">
						<div class="form-group col-md-3">
							<div class="input-group align-items-center" style="height: 40px;">
								...
								<select class="form-control" style="height: 40px; margin-left: 5px;" id="area" name="area" onchange="loadProceso()" required>
									<option value=""></option>
						...
						<div class="form-group col-md-6">
							<div class="input-group align-items-center" style="height: 40px;">
								...
								<input type="text" class="form-control" style="height: 40px;" id="responsable" name="responsable" value="<?php echo $documentos['responsable']; ?>" required>
							</div>
						</div>	
						<div class="form-group col-md-3">
							<div class="input-group align-items-center" style="height: 40px;">
						...
						<div class="form-group col-md-6">
							<div class="input-group align-items-center" style="height: 40px;">
								...
								<input type="text" class="form-control" style="height: 40px;" id="descripciondocumento" name="descripciondocumento" value="<?php echo $documentos['descripcion_documento']; ?>" required>
							</div>
						</div>				
						<div class="form-group col-md-3">
							<div class="input-group align-items-center" style="height: 40px;">
								...
								<input type="text" class="form-control" style="height: 40px;" id="version" name="version" value="<?php echo $documentos['version']; ?>" required>
							</div>
						</div>
						<div class="form-group col-md-3">
							<div class="input-group align-items-center" style="height: 40px;">
								...
								<input type="date" class="form-control" style="height: 40px;" id="fecha" name="fecha" value="<?php echo $documentos['fecha']; ?>" required>
							</div>
						</div>
						<div class="form-group col-md-6">
							<div class="input-group align-items-center" style="height: 40px;">
						...
						<!-- Botón de envío -->
						<div class="form-group col-md-6">
							<button type="submit" name="editar_documento" class="btn btn-success" style="margin-top: 10px; margin-right: 40px; width: 10%; background-color: #28a745;">
								Grabar
							</button>
